...
def pretrain_binary_cwt(
    *,
    ...
    if not dataset_names:
        raise ValueError("No datasets found for binary pretraining.")
    ...
    for data_name in dataset_names:
        ...
        print(
            f"[pretrain:{data_name}] cache train={'hit' if train_cache_hit else 'build'} ({train_cache_path.name}), "
            f"test={'hit' if val_cache_hit else 'build'} ({val_cache_path.name})"
        )
        ...
        run_config = {
            **config,
            ...
            "train_cache_hit": train_cache_hit,
            "test_cache_hit": val_cache_hit,
            "warm_start": latest_state_dict is not None,
        }
        ...
        if wandb_enabled:
            wandb_run = init_wandb_run(
                ...
                entity=wandb_entity,
                group=wandb_group or result_stem,
                tags=(wandb_tags or []) + [result_stem, data_name, "pretrain"],
                notes=wandb_notes,
                mode=wandb_mode,
                run_name=f"{result_stem}-{data_name}",
            )
        ...
        try:
            model = model_builder(latest_state_dict)
            model, latest_state_dict = train_binary_model(
                model,
                train_loader,
                device,
                epochs=epochs,
            ...
            metrics = evaluate_binary_model(model, val_loader, device)
            result = make_result_dict(data_name, model_name, metrics)
            pretrain_results.append(result)
            if wandb_run is not None:
                wandb_run.summary["result"] = result
        finally:
            if wandb_run is not None:
    ...
    torch.save(latest_state_dict, weights_path)

    run_dir = create_run_dir(result_stem, runs_root)
    save_run_summary(run_dir, pretrain_results, config)
    append_results(results_dir, result_stem, pretrain_results)
    return latest_state_dict, pretrain_results, run_dir
